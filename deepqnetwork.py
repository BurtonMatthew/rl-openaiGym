...
class DeepQNetwork:
    def __init__(self, stackSize, numOutputs):
    ...
    def apply(self, frameStack):
        preparedStack = (numpy.stack(frameStack, axis=2) / 255.0).astype(numpy.float32)
        print(preparedStack.shape)
        print(preparedStack.dtype)
        return self.model.apply(preparedStack)
